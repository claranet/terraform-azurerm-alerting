...
| action\_group\_logic\_app\_receiver | Map of Logic Apps to notify. Example: `{ ml-devops = { resource_id = '/subscriptions/xxxx/resourceGroups/xxxx/providers/Microsoft.Logic/workflows/xxxx', callback_url = 'https://xxxx' } }`. | <pre>map(object({<br/>    resource_id  = string<br/>    callback_url = string<br/>  }))</pre> | `{}` | no |
...
| existing\_action\_groups | List of existing Action Groups to associate with alerts. | <pre>list(object({<br/>    id                 = string<br/>    webhook_properties = optional(map(string))<br/>  }))</pre> | `[]` | no |
